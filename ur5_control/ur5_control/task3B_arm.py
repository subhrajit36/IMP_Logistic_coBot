...
class MoveIt2Servo:
    ...
    def __init__(
        ...
        # Create a subscriber for ArUco marker coordinates
        self._target_position = [0.0, 0.0, 0.0]  # Initialize the target position
        self._node.create_subscription(
            Point,
            "/marker_coordinates",
            self._marker_coordinates_callback,
    ...
    def _marker_coordinates_callback(self, msg: Point):
    
        # Callback function to update the target position based on received ArUco marker coordinates.
        # :param msg: Point message containing the x, y, z coordinates of the marker.
    
        self._node.get_logger().info(f"Received marker coordinates: x={msg.x}, y={msg.y}, z={msg.z}")
        self._target_position = [msg.x, msg.y, msg.z]

    def move_to_current_marker(self, tolerance: float = 0.01):
        """
        Move the end-effector to the current marker position based on the last received coordinates.
        :param tolerance: Distance tolerance to stop servoing.
        """
        if self._target_position == [0.0, 0.0, 0.0]:
            self._node.get_logger().warn("No marker coordinates received yet.")
            return

        self._node.get_logger().info(f"Moving to marker position: {self._target_position}")
        self.move_to_marker(target_position=tuple(self._target_position), tolerance=tolerance)

    def __enable_done_callback(self, future: Future):
        """
        Callback executed when the enable service call is completed.
        """
    ...
    def __disable_done_callback(self, future: Future):
        """
        Callback executed when the disable service call is completed.
        """
        result = future.result()
        self.__is_enabled = not result.success
        if not result.success:
...
def main(args=None):
    ...
    node = Node("moveit2_servo_node")
    servo = MoveIt2Servo(node=node, frame_id="base_link")
    
    try:
        node.get_logger().info("Starting servo...")
        servo.enable(sync=True)
        servo.move_to_current_marker()
